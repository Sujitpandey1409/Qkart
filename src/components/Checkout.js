...
const Checkout = () => {
  ...
  const AddNewAddressView = () => {
    return (
      <Box display="flex" flexDirection="column">
        {/* <TextField onChange={(e)=>{setNewAddress({...newAddress,value:e.target.value})}} value={newAddress.value} */}
        ...
        <Stack direction="row" my="1rem">
          ...
          <Button onClick={() => setNewAddress({ ...newAddress, isAddingNewAddress: false })}
            variant="text"
          >
            Cancel
          </Button>
  ...
  const addAddress = async () => {
    if (inputRef.current.value) {
      try {
        ...
        const requestData = { address: inputRef.current.value }
        const response = await axios.post(`${config.endpoint}/user/addresses`, requestData, {
          headers: {
  ...
  const deleteAddress = async (e) => {
    // const addressId = addresses.selected
    const addressId = e.target.parentElement.id
    try {
  ...
  const validateRequest = () => {
    if (!addresses.selected) {
      enqueueSnackbar('Select one shipping address', {
        variant: "error",
      })
      return false
    }
    if (localStorage.getItem('balance') == 0 || localStorage.getItem('balance') < getTotalCartValue(items)) {
  ...
  const performCheckout = async () => {
    if (validateRequest(addAddress)) {
      const requestData = { addressId: addresses.selected }
      try {
        // TODO: CRIO_TASK_MODULE_CHECKOUT - Delete selected address from the backend and display the latest list of addresses
        const response = await axios.post(`${config.endpoint}/cart/checkout`, requestData, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
        ...
        if (e.response) {
          ...
          enqueueSnackbar(
            "Could not place this order. Check that the backend is running, reachable and returns valid JSON.",
            {
              variant: "error",
            }
          );
        ...
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              {addresses &&
                addresses.all.map((el) => (
                  <Typography onClick={() => { setAddresses({ ...addresses, selected: el._id }); }}
                    key={el._id} id={el._id}
                    className={`address-item ${el._id === addresses.selected ? "selected" : ""}`} >
                    {el.address}
                    <button role="button" aria-label="Delete Item" id={el._id} onClick={deleteAddress}><Delete id={el._id}  /></button>
                  </Typography>
                ))}
            </Box>
